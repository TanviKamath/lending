...
Both replace Builder pages -- `portal.build.applications_page` and
`portal.build.application_page` -- and read the same two endpoints.
...
def build_detail():
	...
	return upsert_page(
		...
		frame(
			DETAIL_SOURCE,
			detail_content(read),
			action_label="Back to applications",
			action_route="/borrower/applications",
		),
		[
			api_resource(
				DETAIL_SOURCE,
