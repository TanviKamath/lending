...
--------------------
`portal.build` lays the portal out as Builder pages; this one lays the same portal out
as Studio pages. Both read the same endpoints -- `lending.portal.*`, unchanged -- so
this is a re-layout rather than a rewrite: what moved is the thing that draws the
payload, not the thing that produces it.

Nothing here touches Builder. The Builder pages keep their routes (/borrower/*, /apply,
/track), their exported JSON under lending/builder_files/, and their generators under
portal/build. The Studio app answers on /borrower-portal instead, so the two run side by
side and a cutover is a decision somebody makes later rather than one this migration
makes for them.
...
def build():
	...
	It is not, however, meant to be run twice for the same reason `portal.build` is
	not. Once a page has been opened in Studio and saved, the canvas owns it: Studio
	exports every save to lending/studio/borrower_portal/, and re-running this would
	discard whatever was laid out there.
	"""
	app.upsert_app()
	shell.upsert_frame()
